...
//
//   FUNCTION: CServiceBase::ServiceMain(DWORD, PSTR *)
//
...
//
//   FUNCTION: CServiceBase::CServiceBase(PSTR, BOOL, BOOL, BOOL)
//
...
//
//   FUNCTION: CServiceBase::Start(DWORD, PSTR *)
//
...
void CServiceBase::Start(DWORD dwArgc, PSTR *pszArgv)
{
    ...
    {
        // Log the error.
        WriteEventLogEntry(EVENTLOG_ERROR_TYPE, "Service failed to start.");
...
//
//   FUNCTION: CServiceBase::OnStart(DWORD, PSTR *)
//
...
void CServiceBase::Stop()
{
    ...
    {
        // Log the error.
        WriteEventLogEntry(EVENTLOG_ERROR_TYPE, "Service failed to stop.");
...
void CServiceBase::Pause()
{
    ...
    {
        // Log the error.
        WriteEventLogEntry(EVENTLOG_ERROR_TYPE, "Service failed to pause.");
...
void CServiceBase::Continue()
{
    ...
    {
        // Log the error.
        WriteEventLogEntry(EVENTLOG_ERROR_TYPE, "Service failed to resume.");
...
void CServiceBase::Shutdown()
{
    ...
    {
        // Log the error.
        WriteEventLogEntry(EVENTLOG_ERROR_TYPE, "Service failed to shut down.");
    }
}
...
//
//   FUNCTION: CServiceBase::WriteEventLogEntry(PSTR, WORD)
//
...
void CServiceBase::WriteEventLogEntry(WORD wType, LPCSTR pszFmt, ...) {
    va_list args;
    va_start(args, pszFmt);
    vWriteEventLogEntry(wType, pszFmt, args);
    va_end(args);
}

void CServiceBase::vWriteEventLogEntry(WORD wType, LPCSTR pszFmt, va_list args) {
    HANDLE hEventSource = NULL;
    LPCSTR lpszStrings[2] = { NULL, NULL };

    hEventSource = RegisterEventSourceA(NULL, m_name);
    if (hEventSource)
    {
        CHAR szMessage[260];
        vsnprintf(szMessage, ARRAYSIZE(szMessage), pszFmt, args);
...
void CServiceBase::WriteErrorLogEntry(LPCSTR pszFunction, DWORD dwError) {
    WriteEventLogEntry(EVENTLOG_ERROR_TYPE, "%s failed w/err 0x%08lx", pszFunction, dwError);
}
